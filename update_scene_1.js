...
let content = fs.readFileSync('src/app/maglev/page.tsx', 'utf-8');

const new_scene = `    // 1. Scene Setup - HIGHWAY LANDSCAPE
...
content = content.replace(
    /\/\/ 1\. Scene Setup[\s\S]*?renderer\.dispose\(\);\n\s*\};\n\s*\}, \[\]\);/,
    new_scene + "\n  }, []);"
);

content = content.replace(
    /\{\/\* Callouts over the canvas replicating the image \*\/\}[\s\S]*?\{\/\* Exploded Coil Model \*\/\}/,
    '{/* Exploded Coil Model */}'
);

fs.writeFileSync('src/app/maglev/page.tsx', content, 'utf-8');
console.log("Successfully injected new Scene 1 into page.tsx");
