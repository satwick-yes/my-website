...
with open('src/app/maglev/page.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

new_scene = """    // 1. Scene Setup - HIGHWAY LANDSCAPE
    const scene = new THREE.Scene();
    scene.background = new THREE.Color("#9ab8ce"); // Bright sky
    scene.fog = new THREE.FogExp2("#9ab8ce", 0.015);
    ...
    const makeLabelSprite = (text: string, bgColor: string, textColor: string, w = 512, h = 128, fs = 32) => {
        const c = document.createElement('canvas'); c.width = w; c.height = h;
        const ctx = c.getContext('2d')!;
        ctx.fillStyle = bgColor; ctx.fillRect(0,0,w,h);
        ctx.strokeStyle = "#444"; ctx.lineWidth = 4; ctx.strokeRect(0,0,w,h);
        ctx.fillStyle = textColor; ctx.font = `Bold ${fs}px sans-serif`;
    ...
    const lblLF = makeLabelSprite("LF Component (20-40 Hz)", "rgba(40, 40, 40, 0.9)", "#ff3333", 550, 80, 26); lblLF.position.set(-8, 0, 26); scene.add(lblLF);
    const lblHF = makeLabelSprite("HF Component (85 kHz)", "rgba(40, 40, 40, 0.9)", "#ffcc00", 550, 80, 26); lblHF.position.set(-8, -1.2, 26); scene.add(lblHF);
...
content = re.sub(
    r'// 1\. Scene Setup.*?renderer\.dispose\(\);\n\s*\};\n\s*\}, \[\]\);',
    new_scene + "\n  }, []);",
    content,
    flags=re.DOTALL
)

# Remove HTML old callouts for canvas 1
content = re.sub(
    r'\{/\* Callouts over the canvas replicating the image \*/\}.*?\{/\* Exploded Coil Model \*/\}',
    r'{/* Exploded Coil Model */}',
    content,
    flags=re.DOTALL
)

with open('src/app/maglev/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
